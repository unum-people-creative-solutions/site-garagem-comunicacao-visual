# AGENTS.md — site-garagem-comunicacao-visual

Operado sob **TLC 2.0 + tlc-spec-driven v3**. Fases, auto-sizing, Sub-Agent Delegation e Verifier estão na skill — consulte o AGENTS.md da raiz do workspace para contexto completo de projeto.

## 🛡️ Regras e Harness
Leia `unum-people-docs/spec/RULES.md` antes de qualquer implementação.

## 📂 Specs e Tech Designs
NUNCA criar `.specs/`, `.spec/` ou `spec/` local neste repo — é o default da skill `tlc-spec-driven`, mas não se aplica aqui. Specs e tech-designs deste projeto vivem em `unum-people-docs` (repo separado, sibling deste): `unum-people-docs/spec/features/` e `unum-people-docs/tech-designs/`. Ver pastas existentes lá para o nome de projeto correto antes de criar uma nova (features cross-cutting Backend+Frontend costumam usar `unum-people-services/`).

## 📝 Contexto Local
Next.js 14, Framer Motion.

## 🛠️ Decisões Ativas
- **Performance**: Hero desativa animações pesadas via `usePerformanceTier` (Graceful Degradation 3 tiers). Animações de loop infinito em `globals.css` com `will-change: transform, opacity` (GPU).
- **Scroll Lock**: `BeforeAfterSlider` ativa `touch-none` + `overflow: hidden` no `body` durante manipulação.
- **Footer**: Layout centralizado para evitar sobreposição com botão flutuante do WhatsApp. Hover do bloco de créditos acende simultaneamente em `accent` (referência: `site-psicologa-andrielly`).
- **Social**: Dois perfis do Instagram no rodapé (`@garagemcomunicacaovisuall`).
- **Services**: Lista atualizada (Fachadas, Wind Banners, Cavaletes, Plotagem, Banners, Papelaria) mantendo design original.
